			...
			<div>
				<div class="container">
					...
					<c:if test="${user != 'anonymousUser' && user.auth == 'ADMIN'}">
						<a href="/notice/write" class="btn btn-outline-warning" id="admit">게시글
							등록</a>
					</c:if>
				</div>
					...
					<ul class="pagination">
						<li class="page-item ${paging.prev ? '' : 'disabled'}"><c:choose>
								<c:when test="${paging.startPage == 1}">
									<a class="page-link"
										href="/notice/list?page=${paging.startPage=1}">Previous</a>
								</c:when>
								<c:otherwise>
									<a class="page-link"
										href="/notice/list?page=${paging.startPage-1}">Previous</a>
						...
						<li class="page-item ${paging.next ? '' : 'disabled'}"><c:choose>
								<c:when test="${paging.endPage < 10}">
									<a class="page-link"
										href="/notice/list?page=${paging.endPage=paging.endPage}">Next</a>
								</c:when>
								<c:otherwise>
									<a class="page-link"
										href="/notice/list?page=${paging.endPage + 1}">Next</a>
								</c:otherwise>
							</c:choose></li>
					</ul>
				</nav>
