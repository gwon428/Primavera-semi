...
<div class="tabs">
  ...
  <div class="tab-content">
      ...
      <span>* 수정 불가 항목입니다.</span>
       <session id="user">
	      <session class="session">
	      	  <input type="hidden" name="orderNum" id="orderNum" value="${collect.orderNum}" readonly/>
		      <h2>아이디</h2>
		      <input type="hidden" name="id" id="id" value="${collect.id}" readonly/>
		      <p>${collect.id}</p>
		      <h2 class="m_top">이름</h2>
		      <input type="hidden" name="name" id="name" value="${collect.name}" readonly/>
		      <p>${collect.name}</p>
	      </session>
	      <session>
		      <h2>전화번호</h2>
		      <input type="hidden" name="phone" id="phone" value="${collect.phone}" readonly/>
		      <p>${collect.phone}</p>
	     </session>
    ...
    <div class="contents">
      <i class="fa-solid fa-location-dot"></i>
       <session id="user" style="flex-direction:column">
	      <session>
		     <h2>우편번호</h2>
		     <input type="text" name="postCode" id="postCode" placeholder="우편번호" size="15"  value="${collect.postCode}"/>
		     <input type="button" onclick="daumPostCode()" value="우편번호 찾기" /> 
	      </session>
	      <session class="session">
	      
		     <h2 style="margin-top : 15px">주소</h2>
		     <input type="text" name="roadAddress" id="roadAddress" placeholder="주소" size="40" value="${collect.roadAddress}"/>
			 <h2 style="margin-top : 19px">상세주소</h2>
			 <input type="text" name="detailAddress" id="detailAddress" placeholder="상세 주소" size="40"  value="${collect.detailAddress}"/>
	     </session>
     </session>
    ...
    <div class="contents">
      ...
      <h2>수거일</h2>
      <input type="text" name="collectionDate" id="collectionDate" value="${collect.collectionDate}"/>
      <h2 class="m_top">수거 신청 무게</h2>
      <input type="number" name="kg" id="kg" placeholder="5~20kg 신청 가능" min="5" max="20" value="${collect.kg}"/>
